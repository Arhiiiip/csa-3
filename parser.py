...
functions = ['setq', 'defun', 'read', 'print', 'loop', 'while', 'if', '+', '-', '>=', '>', '<', '<=', '=', '/', '!=', '%', 'return', 'continue']
...
def convert_list_to_s_expression(lisp_list):
        ...
        if args[0] == 'for':
            ...
            args += split_code_into_terms(
                add_spaces(f'(setq {iter_var} (+ {iter_var} 1))'
                           f'(if (> {iter_var} {iter_span[1]}) (continue) (return))'
                           )
            )
    ...
    if s_expression.operator == 'defun':
        functions.append(s_expression.args[0])
...
ans = read("(setq n 2520) (setq i 20) (while (> i 0) (if (= (% n i) 0) (setq i (- i 1)) (setq x 0))"
